...
for x in depth.Aphrocallistes_reference_assembly_v1_ONT-RNA_minimap2_renamed.bam ;do
...
#for file in *gc.txt ; do sed 's/000/' $file | sed -E 's/ONT_TR_PE_scaff_([0-9]{2})	/ONT_TR_PE_scaff_0\1	/' | sed -E 's/ONT_TR_PE_scaff_([0-9]{1})	/ONT_TR_PE_scaff_00\1	/' | sort -n > ${file%.txt}_renamed.txt ; done
